...
async function composeDevSupergraph(productsUrl, productsSchema) {
    ...
    await composeDevSubgraph('users', 'http://localhost:4002', (0, path_1.resolve)(__dirname, 'subgraphs', 'users', 'users.graphql'));
    await composeDevSubgraph('inventory', 'http://localhost:4003', (0, path_1.resolve)(__dirname, 'subgraphs', 'inventory', 'inventory.graphql'));
...
async function composeSupergraph(schemaFile, graphQLEndpoint = '', port = '4001') {
    ...
    const template = await (0, promises_1.readFile)((0, path_1.resolve)(__dirname, '../supergraph-config.yaml.template'), 'utf-8');
    const supergraphConfig = template
        .replaceAll('${DIST_DIR}', (0, path_1.resolve)(__dirname))
        .replace('${PORT}', port)
